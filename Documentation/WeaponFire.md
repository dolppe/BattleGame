...
- 추후 진행할 예정이었던 해결 방법  
    - GCNParams를 상속 받아서 Array를 처리하도록 함.  
    커스텀한 GCN Params에 값을 세팅하고, 이를 업캐스팅하여 여러 함수를 처리하고, 추후에 다운 캐스팅하는 방법.
    - GCN_Burst_Array를 만들고, 내부 변수를 세팅.
    GCN_Burst_Array라는 GCN_Burst를 상속받아서 커스텀한 클래스를 만들고, 해당 클래스 내부에 Array를 받을 수 있도록 하는 방법.

- 각 방법의 문제점
    - GCNParams 상속  
        - 문제점 1: 기존의 GCNParams에 상속이 고려되지 않았음.  
        해당 구조체는 모든 변수, 함수들이 public으로 열려있기 때문에, 이를 상속해서 Array를 넣는 것은 문제가 되지 않기는 함.  
        그러나, virtual 함수도 하나도 없고, proteced처리되거나 뭔가 상속에 대한 내용이 하나도 없음.  
        UE 제작자가 이에 대해 상속을 고려 했는지를 모르겠어서, 진행하지 않기로 함.
        - 문제점 2: 최적화 관련.  
        위의 임시 조치 로직은 GCN에서 진행하던 작업을 GA에 옮겼을 뿐, 기존의 로직과 비교해서 가짜 총알궤적을 만드는 부분을 오히려 없애서 성능적으로는 더 좋을 것임.  
        총을 쏘는 경우가 자주 일어나는 만큼 성능이 중요하다고 생각함.  
        그런데, GCNParams를 상속 받는다면, 구조적으로는 이펙트를 GCN에서 실행시키기 때문에 맞는 것 같기는 함.  
        그러나, 매번 총을 쏠 때마다 두 번의 캐스팅이 일어나야하고, 추가적으로 클래스를 체크하거나 하는 등의 작업이 필요할 수 있기 때문에 위의 임시 조치로 한 방법보다 추가적인 성능이 필요함.  
        - 결론  
        GCNParams에 상속과 관련된 것이 고려되지 않았다는 점과 자주 사용되는 행동에 대해서 추가적인 성능이 필요하다는 점을 생각해보면, 이는 기존의 방법으로 진행하는 것이 좋겠다는 결론을 냈음.
    - GCN_Burst 상속
